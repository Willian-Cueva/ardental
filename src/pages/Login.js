...
export default function Login() {
  ...
  const loginLocal = async () => {
    setShowModal(true);
    const d = document,
      $email = d.getElementById("id-inp-email-login"),
      $password = d.getElementById("id-inp-password-login");
    ...
    await loginUser(user)
      .then((data) => {
        if (data.status !== "ok") {
          ...
          console.log("Si nos logeamos", data.data);
          if (login(data.data)) {
    ...
    <main className={` w-full bg-[#E3F2FD] ${styles.main} p-8`} id="id-login">
      ...
      <section className="max-w-[473px] bg-white rounded-xl border border-[#90caf983] ml-auto mr-auto p-[24px] flex flex-col items-center justify-center">
        ...
        <div className="w-full flex justify-end">
          <Link
            to={"/forgot-password"}
            className={`${styles.forgotPassword} mb-4 hover:underline`}
          >
